...
public class AutoGenerateQuizServlet extends HttpServlet {
    ...
    /**
     * 主要處理邏輯：從 影片文字（transcript）呼叫 GPT 產生選擇題 → 寫入資料庫 → 回傳 JSON
     */
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        String prompt = "請僅回傳 JSON 陣列，不要加上任何註解或文字。" +
        	    "請產生 5 題繁體中文選擇題，不要簡體中文，語意要清楚、適合 Java 初學者的測驗題目。" +
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            for (int i = 0; i < quizArr.length(); i++) {
                ...
                String explanation = q.has("explanation") ? q.getString("explanation") : "無提供";
    ...
    private String callOpenAIGPT(String prompt) throws IOException {
    	...
    	OkHttpClient client = (OkHttpClient) context.getAttribute("sharedOkHttpClient");

    	if (client == null) {
    	    System.out.println("⚠️ 找不到共用 OkHttpClient，改為 new 一個（不建議）");
    	    client = new OkHttpClient(); // fallback
    	}
